...
const ListPage = () => {
  ...
  const getStatusBadge = status => {
    ...
    return (
      <span
        className={`px-2 py-1 text-xs rounded-full ${statusStyles[status] || statusStyles.draft}`}
      >
        {status}
      </span>
    );
      ...
      {surveys.length === 0 ? (
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">Your Surveys ({surveys.length})</h2>
          </div>
              ...
              <div key={survey.oid} className="p-6 hover:bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    {survey.description && (
                      <p className="mt-1 text-gray-600 text-sm">{survey.description}</p>
                    )}
                    ...
                    <div className="mt-2 flex items-center space-x-4 text-sm text-gray-500">
                      ...
                      {survey.category_display && <span>{survey.category_display}</span>}
                    </div>
                  </div>
