...
import { Calendar, CheckCircle, Clock, Edit, Users, XCircle } from 'lucide-react';
...
const Header = ({ survey, formatDate }) => {
  ...
  const getStatusBadge = status => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusStyles[status] || statusStyles.draft}`}
      >
